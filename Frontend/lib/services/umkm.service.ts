...
export async function getUmkmList(kategori?: string): Promise<Umkm[]> { return apiRequest<Umkm[]>(`/umkm${query({ kategori })}`); }
export async function getUmkmBySlug(slug: string): Promise<Umkm | null> { try { return await apiRequest<Umkm>(`/umkm/${encodeURIComponent(slug)}`); } catch (error) { if (error instanceof ApiError && error.status === 404) return null; throw error; } }
