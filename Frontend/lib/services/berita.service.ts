...
import type { Berita, BeritaListParams, KategoriBerita, PaginatedResult } from '@/types/berita';
...
export async function getBeritaList(params: BeritaListParams = {}): Promise<PaginatedResult<Berita>> {
  const response = await apiRequest<BeritaListResponse>(`/berita${query({ kategori: params.kategori, search: params.search, page: params.page, limit: params.perPage })}`);
  return asPaginated(response);
}

export async function getBeritaBySlug(slug: string): Promise<Berita | null> {
  try { return await apiRequest<Berita>(`/berita/${encodeURIComponent(slug)}`); }
...
export interface BeritaEditableInput { judul: string; ringkasan: string; konten: string; kategori: KategoriBerita; gambarSampul: string; tags?: string[]; }
